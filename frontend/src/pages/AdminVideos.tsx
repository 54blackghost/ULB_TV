...
import { getAllAdminVideos, deleteAdminVideo } from '../services/videoService';
...
const AdminVideos: React.FC = () => {
  ...
  if (loading) {
    return <div className="text-black dark:text-white">Loading videos...</div>;
  }
  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link to="/admin/videos/new" className="inline-flex items-center justify-center rounded-md bg-primary py-2 px-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10">
          Create New Video
        </Link>
      </div>

      {videos.length === 0 ? (
        <p className="text-black dark:text-white">No videos found.</p>
      ) : (
                ...
                {videos.map((video) => (
                  <tr key={video._id}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{video.author.name}</p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <Link to={`/admin/videos/edit/${video._id}`} className="hover:text-primary">
                          {/* Edit Icon */}
                        ...
                        <button onClick={() => handleDelete(video._id)} className="hover:text-primary">
                          {/* Delete Icon */}
                          <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
